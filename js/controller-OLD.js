...
function teacherController($scope, $resource, $stateParams, $state, $modal, $log, getStudent) {
    ...
    StudentRecord.query(function(result) {
        // $log.warn('StudentRecord.query called in teacherController, number of students BEFORE:' + result.length);
        $scope.students = result;
        // for (var i = 0; i < $scope.students.length; i++) {
        //     $log.debug('   Student[' + i + '] loaded: ' + $scope.students[i].sid);
        // };
    });
    ...
    $scope.viewStudentRecord = function(student) {
        // $log.debug('Number of Student records BEFORE View: ' + $scope.students.length);
        StudentRecord.get({
            id: student.sid
        }, function(result) {
            var studentParams = {
            ...
            // $log.debug('Number of Student records after View: ' + $scope.students.length);
        });
    };
...
function TodayViewController($scope, $resource, $modal, $stateParams, $state, getStudent, $log, $q) {
    ...
    StudentRecord.query(function(result) {
        // $log.warn('StudentRecord.query called in TodayViewController, number of students BEFORE:' + result.length);
...
function loginCtrl($state, $stateParams, $scope, $resource, $http, store, jwtHelper, getTeacherProfile, $log) {
        ...
        $http.post('/api/signin', {
            ...
            .success(function(data, status, header, config) {
                ...
                TeacherRecord.get({
                    ...
                    $scope.teacherProfile = result;
                    $log.warn('Result of TeacherRecord.get (firstName): ' + result.firstName);
                    // getTeacherProfile.initializeTeacherData(result);
                });
                $state.go('teacher-dashboard.main');
            })
 ...
 *       Factory that passes student data from one controller to another
 */
.service('getTeacherProfile', [function() {
    var teacherProfileData = {};

    var initializeTeacherData = function(teacherSent) {
        teacherProfileData = teacherSent;
    }

    var getTeacherProfileData = function() {
        return teacherProfileData;
    }

    return {
        initializeTeacherData: initializeTeacherData,
